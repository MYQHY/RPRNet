...
def boxes_to_heatmap(pred_boxes,pred_conf,H, W, sigma=3,normalize=True):
    """
    将预测的 boxes (cx, cy, w, h) 和 conf 直接转换为加权高斯热图。

    Args:
        pred_boxes: [B, T, Q, 4]，中心点和宽高归一化到 [0,1]
        pred_conf:  [B, T, Q, 1]，置信度（未sigmoid也可）
        H, W: 输出热图大小
        sigma: 高斯半径（可为标量或张量）
    ...
    xv = xv[None, None, :, :].float()  # [1,1,H,W]
    yv = yv[None, None, :, :].float()
    # 初始化热图
    heatmap = torch.zeros((B, H, W), device=device)

    for q in range(Q):
        # 取出当前 query 的参数
        cx = pred_boxes[..., q, 0:1] * W   # [B,T,1]
        cy = pred_boxes[..., q, 1:2] * H   # [B,T,1]
        w  = pred_boxes[..., q, 2:3] * W
        h  = pred_boxes[..., q, 3:4] * H
        c  = pred_conf[..., q, :]               # [B,T,1]

        # 计算二维高斯
        gx = (xv - cx[..., None, None]) ** 2
        gy = (yv - cy[..., None, None]) ** 2
        g  = torch.exp(-(gx + gy) / (2 * sigma ** 2))  # [B,T,1,H,W]

        # 用置信度加权叠加
        heatmap += c.reshape((B,1,1)) * g.squeeze(1)

    if normalize:
        # 限制范围在 [0,1]
        heatmap = heatmap / (heatmap.amax(dim=(-2,-1), keepdim=True) + 1e-6)
