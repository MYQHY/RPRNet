...
parser.add_argument("--pth_dirs", default=['TSIRMT/RPRNet.pth.tar'], type=list)
parser.add_argument("--dataset_dir", default=r'D:\Code\RPRNet\datasets', type=str, help="train_dataset_dir")
parser.add_argument("--dataset_names", default='TSIRMT', type=str)
parser.add_argument("--img_norm_cfg", default=None, type=dict,
                    help="specific a img_norm_cfg, default=None (using img_norm_cfg values of each dataset)")
parser.add_argument("--save_img", default=True, type=bool, help="save image of or not")
parser.add_argument("--save_img_dir", type=str, default=r'D:\Code\RPRNet\result/',
                    help="path of saved image")
parser.add_argument("--save_log", type=str, default=r'D:\Code\RPRNet\log/', help="path of saved .pth")
parser.add_argument("--threshold", type=float, default=0.5)
parser.add_argument("--num_frame", type=int, default=6)
parser.add_argument("--patchSize", type=int, default=256)
global opt
opt = parser.parse_args()

def test():
    test_set = seqDataset(dataset_dir=opt.dataset_dir, dataset_name=opt.dataset_names, patch_size=opt.patchSize,
    ...
    # 计算PD_FA   完全OK
    eval_05 = PD_FA()
    ROC_05 = ROCMetric05(nclass=1, bins=30)
    config_vit = config.get_SCTrans_config()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # CPU
    net = RPRNet(3, opt.num_frame).to(device)
    ...
    with torch.no_grad():
        for idx_iter, (img, gt_mask, [ori_w,ori_h]) in enumerate(tbar):
            # img = Variable(img)
            img = img.to(device, non_blocking=True)
            gt_mask = gt_mask.to(device, non_blocking=True)
            # CPU
            output = net.forward(img)
            pred = output["main_logit"]
            ...
            eval_05.update((pred[0, 0, :, :] > opt.threshold).cpu(), gt_mask[0, 0, :, :], [ori_w,ori_h])  # 目标
            ROC_05.update(pred, gt_mask)
            # save img
            if opt.save_img == True:
                img_save = transforms.ToPILImage()((pred[0, 0, :, :]).cpu())
                mask_save = transforms.ToPILImage()(((gt_mask[0, 0, :, :]>0).float()).cpu())
                if not os.path.exists(opt.save_img_dir + opt.test_dataset_name + '/' + opt.model_name):
                    os.makedirs(opt.save_img_dir + opt.test_dataset_name + '/' + opt.model_name)
                if not os.path.exists(opt.save_img_dir + opt.test_dataset_name + '/' + opt.model_name + '/pred'):
                    os.makedirs(opt.save_img_dir + opt.test_dataset_name + '/' + opt.model_name + '/pred')
                if not os.path.exists(opt.save_img_dir + opt.test_dataset_name + '/' + opt.model_name + '/gt'):
                    os.makedirs(opt.save_img_dir + opt.test_dataset_name + '/' + opt.model_name + '/gt')
                img_save.save(opt.save_img_dir + opt.test_dataset_name + '/' + opt.model_name + '/pred/test' + str(idx_iter) + '.png')
                mask_save.save(opt.save_img_dir + opt.test_dataset_name + '/' + opt.model_name + '/gt/test' + str(idx_iter) + '.png')
                save_dir = opt.save_img_dir + opt.test_dataset_name + '/' + opt.model_name
                pred_np = pred[0, 0,:,:].cpu().numpy()
                gt_np = gt_mask[0, 0,:,:].cpu().numpy()
                savemat(f"{save_dir}/pred/test{idx_iter}.mat", {"pred": pred_np})
                savemat(f"{save_dir}/gt/test{idx_iter}.mat", {"gt": gt_np})
        # 0.5
        pixAcc, mIOU = IOU.get()
        nIoU = nIoU_metric.get()
        results2 = eval_05.get()
        #
        true_positive_rate, false_positive_rate, recall, precision, FP, F1_score = ROC_05.get()
